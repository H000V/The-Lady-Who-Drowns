...
public class GrabHandPose : MonoBehaviour
{
    ...
    public HandData primaryRightHandPose;
    public HandData primaryLeftHandPose;
    public HandData secondaryRightHandPose;
    public HandData secondaryLeftHandPose;
    ...
    public void SetupPose(BaseInteractionEventArgs arg)
    {
        if(arg.interactorObject is XRDirectInteractor)
        {
            ...
            XRGrabInteractable grabInteractable = arg.interactableObject as XRGrabInteractable;
            Transform attachTransformUsed = arg.interactorObject.transform;

            if (attachTransformUsed == grabInteractable.attachTransform)
            {
                if (handData.handType == HandData.HandModelType.Right)
                {
                    SetHandDataValues(handData, primaryRightHandPose);
                }
                else
                {
                    SetHandDataValues(handData, primaryLeftHandPose);
                }
            }
            else if (attachTransformUsed == grabInteractable.secondaryAttachTransform)
            {
                if (handData.handType == HandData.HandModelType.Right)
                {
                    SetHandDataValues(handData, secondaryRightHandPose);
                }
                else
                {
                    SetHandDataValues(handData, secondaryLeftHandPose);
                }
            }
    ...
    public static void MirrorRightPose()
    {
        ...
        if (handPose.secondaryRightHandPose != null && handPose.secondaryLeftHandPose != null)
        {
